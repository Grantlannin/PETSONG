import type { Brief } from './types';

/** Golden retriever test brief — used to pre-fill /lab intake. */
export const SAMPLE_BRIEF: Brief = {
  pet_name: 'Biscuit',
  nickname: 'Bis',
  pet_type: 'Golden Retriever',
  breed: 'Golden Retriever',
  personality:
    'Gentle, goofy, and stubborn in the sweetest way. On her best day she\'s a sunshine sponge — tail going, eyes soft, convinced every stranger is her new best friend. She\'s dramatic about being left out of a room and will sigh like a teenager if you close a door on her.',
  what_makes_them_them:
    'She always brings you something when you come home — usually a sock, sometimes a shoe, once a whole zucchini from the garden. She sits with her back against your legs while you cook. Every family member has a different nickname for her and she responds to all of them.',
  signature_daily_rituals:
    '6am: nose on the bed until someone gets up. Morning walk is non-negotiable — she grabs the leash herself. Dinner at 5pm sharp; she starts pacing at 4:45. 9pm zoomies around the couch without fail. Sleeps on the rug outside my daughter\'s door every night.',
  memorable_inside_jokes:
    'We call her "The General" when she herds us toward the kitchen. "Biscuit tax" means she gets the last bite of anything. My daughter says she speaks "Biscuit" — a series of grumbles that apparently mean very specific things. The running joke is she thinks she\'s a lap dog.',
  signature_behaviors:
    'Steals socks from the hamper and parades them like trophies. Zoomies at 9pm sharp. Refuses to walk past the mailbox without a full minute sniff inspection. One ear never stands up — folds over like a little flag. Greets everyone by sitting on their feet.',
  favorite_things:
    'Tennis balls (especially the squeaky one she\'s destroyed three times). Peanut butter Kongs. The creek at the end of our street. My daughter — above all else. Sunbeams on the living room rug. Car rides with the window cracked.',
  things_they_hate:
    'The vacuum cleaner — she hides in the bathtub. Baths. Fireworks. Being left alone in the car, even for two minutes. The mail carrier (she\'s convinced it\'s personal). Nail trims — the drama is Oscar-worthy.',
  special_relationships:
    'My daughter\'s first dog — slept outside her crib every night for two years. Protective of the kids but gentle as anything with them. Thinks the cat is her responsibility (the cat disagrees). My shadow on bad days — never more than three feet away.',
  if_they_could_talk:
    '"Did someone say walk? Also I love you. Also did someone say walk?" She\'d describe our family as "mine" and herself as "basically in charge." Catchphrase: a long sigh followed by putting her chin on your knee.',
  quirks_nobody_else_would_know:
    'She only drinks water if you turn the tap on first — bowl water is beneath her. She knows which cabinet the treats are in and will sit in front of it and stare without blinking. When she\'s worried about you she brings her blankie.',
  emotional_moments:
    'The night I came home crying from the hospital, she dragged her blankie to the couch and put her chin on my knee until I stopped shaking. She got me through my divorce — when I couldn\'t get out of bed, she\'d sit on the floor and stare at me like "I\'m still here." Twelve years of showing up.',
  occasion: 'birthday',
  vibe: 'heartfelt',
};
